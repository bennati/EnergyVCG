...
class TestMeasures(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        f=functools.partial(BaseMeasurementGen,N)
        self.s=BaseSupervisor(N,decision_fct=DecisionLogicSupervisorTesting,agent_decision_fct=DecisionLogicTesting,measurement_fct=f)
    ...
    def test_decentralized_always_contrib(self):
        ...
        rewards=[{"reward":0} for i in range(self.s.N)]
        measures=self.s.evaluate(decs,rewards)[0]
    ...
    def test_decentralized_always_contrib_welfare(self):
        ...
        decs=self.s.decisions(percs)
        rewards=[{"reward":n2} for i in range(self.s.N)] # same reward for all
    ...
    def test_decentralized_always_contrib_efficiency(self):
        ...
        decs=self.s.decisions(percs)
        measures=self.s.evaluate(decs,rewards,threshold=self.s.N)[0] # the threshold is the number of contributors
        ...
        measures=self.s.evaluate(decs,rewards,threshold=self.s.N//2)[0] # the threshold is half the number of contributors
    ...
    def test_decentralized_always_contrib_success(self):
        ...
        measures=self.s.evaluate(decs,rewards,threshold=thresh)[0] # the threshold is the number of contributors
        self.assertEqual(measures["success"],1)
        # more than enough contribute
        measures=self.s.evaluate(decs,rewards,threshold=thresh-1)[0] # the threshold is lower than the number of contributors
        self.assertEqual(measures["success"],1)
        # less than enough contribute
        measures=self.s.evaluate(decs,rewards,threshold=thresh+1)[0] # the threshold is hicher than the number of contributors
...
class DecisionLogicTesting(BaseDecisionLogic):
    ...
    def get_decision(self,perceptions):
        self.act={"contribution":perceptions["value"],"cost":perceptions["cost"],"contributed":True}
        return self.act
